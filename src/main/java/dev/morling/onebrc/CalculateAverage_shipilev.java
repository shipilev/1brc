...
public class CalculateAverage_shipilev {
    ...
    public static class B {
        @Benchmark
        @Fork(value = 1, jvmArgsAppend = { "-XX:+UnlockExperimentalVMOptions", "-XX:+UseEpsilonGC", "-Xms1g", "-Xmx1g", "-XX:+AlwaysPreTouch",
                "-XX:-TieredCompilation", "-XX:CICompilerCount=1", "-XX:-UseCountedLoopSafepoints", "-XX:+TrustFinalNonStaticFields" })
        @BenchmarkMode(Mode.AverageTime)
        @Warmup(iterations = 1, time = 1)
        @Measurement(iterations = 5, time = 1)
        @OutputTimeUnit(TimeUnit.SECONDS)
        public String test(Blackhole bh) throws Exception {
            return run();
        }
    }
